...
//////     test add details to trace button    /////
test('LTLSIM - 16', async () => {
    // console.log('starting test '+numTest+': LTLSIM - 16');
    await cpReferenceDB('LTLSIM_1');
...
//////     test add details to trace button    /////
test('LTLSIM - 17', async () => {
    // console.log('starting test '+numTest+': LTLSIM - 17');
    await cpReferenceDB('LTLSIM_1');
...
test('LTLSIM - 18', async () => {
    // console.log('starting test '+numTest+': LTLSIM - 18');
    await cpReferenceDB('LTLSIM_1');
...
test('LTLSIM - 19', async () => {
    // console.log('starting test '+numTest+': LTLSIM - 19');
    await cpReferenceDB('LTLSIM_1');
...
test('LTLSIM - 20', async () => {
    // console.log('starting test '+numTest+': LTLSIM - 20');
    await cpReferenceDB('LTLSIM_1');
...
test('LTLSIM - 21', async () => {
    // console.log('starting test '+numTest+': LTLSIM - 21');
    await startWithJsonFileImport('FSM-Demo.json');
...
test('GL - 6', async () => {
    ...
    const testReqs = await window.locator('#qa_proj_select_TestRequirements');
    await testReqs.click();

    const mtpReq = await window.locator('#qa_cirPack_text_MetricTemporalProperty');
    await mtpReq.click();
    ...
    const comp = await window.locator('#qa_gls_mi_comp_AUTOPILOT');
    await comp.click();

    var pVar = await window.locator('#qa_gls_ti_var_initialization');
    await pVar.click();

    var varReqs = await window.locator('#qa_gls_ti_var_reqs_initialization');
    var varReqsTxt =  await varReqs.allInnerTexts();
    //console.log('variable in reqs: ', varReqsTxt);
    expect(varReqsTxt).toContain('reqs:\n\nTesting after');

    pVar = await window.locator('#qa_gls_ti_var_m1');
    await pVar.click();

    varReqs = await window.locator('#qa_gls_ti_var_reqs_m1');
    var varReqsTxt =  await varReqs.allInnerTexts();
    //console.log('variable in reqs: ', varReqsTxt);
    expect(varReqsTxt).toContain('reqs:\n\nTestRequirement');


    pVar = await window.locator('#qa_gls_ti_var_p');
    await pVar.click();

    varReqs = await window.locator('#qa_gls_ti_var_reqs_p');
    var varReqsTxt =  await varReqs.allInnerTexts();
    //console.log('variable in reqs: ', varReqsTxt);
    expect(varReqsTxt).toContain('reqs:\n\nTestRequirement');

    pVar = await window.locator('#qa_gls_ti_var_xin');
    await pVar.click();

    varReqs = await window.locator('#qa_gls_ti_var_reqs_xin');
    var varReqsTxt =  await varReqs.allInnerTexts();
    //console.log('variable in reqs: ', varReqsTxt);
    expect(varReqsTxt).toContain('reqs:\n\nTesting after');
...
//------------------------------------------------------------------
test('ASSISTANT TAB - MULTIPLE FORMALIZATIONS FUTURE/PAST DISABLED', async () => {
     console.log('starting test '+numTest+':  ASSISTANT TAB - MULTIPLE FORMALIZATIONS FUTURE/PAST DISABLED')

     electronApp = await electron.launch({ args: [path.join(__dirname, '../../app')] });
     const window = await electronApp.firstWindow();

     // click on the create button to add a requirement
     const crtBtn =  window.locator('#qa_db_btn_create');
     await crtBtn.click();

     const slateEditable =  window.locator('#qa_crt_edt_editor');
     await slateEditable.click();

     await slateEditable.fill('component shall satisfy p');

     await delay(2000);  //just to check
     ...
     const futureTimeFormula = await window.locator('#qa_crtAst_sem_typ_futureTimeFormula');
     var reqText = await futureTimeFormula.textContent();
     expect(reqText).toContain('((! LAST) U p)');

     var switchDisabled =  await window.locator('#qa_crtAst_sem_btn_switchFutureDisabled');
     var checkButton = await switchDisabled.isVisible();
     await delay(1000);  //just to check
     expect(checkButton).toBeTruthy();

     var formatMenu =  await window.locator('#qa_crtAst_sem_sel_future');
     var formatChoice = await formatMenu.isVisible();
     expect(formatChoice).toBeTruthy();

     const pastTime = await window.locator('#qa_crtAst_sem_typ_pastTime');
     await pastTime.click();

     formatMenu =  await window.locator('#qa_crtAst_sem_sel_past');
     var formatChoice = await formatMenu.textContent();
     expect(formatChoice).toBe('SMV');

     const pastTimeFormula = await window.locator('#qa_crtAst_sem_typ_pastTimeFormula');
     reqText = await pastTimeFormula.textContent();
     expect(reqText).toContain('(O p)');

     switchDisabled =  await window.locator('#qa_crtAst_sem_btn_switchPastDisabled');
     checkButton = await switchDisabled.isVisible();
     await delay(1000);  //just to check
     expect(checkButton).toBeTruthy();
});

//------------------------------------------------------------------
test('ASSISTANT TAB - MULTIPLE FORMALIZATIONS FUTURE ENABLED', async () => {
     console.log('starting test '+numTest+':  ASSISTANT TAB - MULTIPLE FORMALIZATIONS FUTURE ENABLED')
     ...
     await slateEditable.fill('after m component shall immediately satisfy p');
     ...
     const formatMenu =  window.locator('#qa_crtAst_sem_mi_futureSmv');
     checkButton = formatMenu.isVisible();
     expect(checkButton).toBeTruthy();

     const futureTimeFormula = await window.locator('#qa_crtAst_sem_typ_futureTimeFormula');
     var reqText = await futureTimeFormula.textContent();
     expect(reqText).toContain('(((! FLin_m) U (FLin_m & (X p))) | (LAST V (! FLin_m)))');

     const switchEnabled =  await window.locator('#qa_crtAst_sem_btn_switchFutureEnabled');
     var checkButton = switchEnabled.isVisible();
     await delay(1000);
     expect(checkButton).toBeTruthy();

     checkButton = await switchEnabled.click();
     await delay(1000);
     reqText = await futureTimeFormula.textContent();
     expect(reqText).toContain('(((! ((m & (! LAST)) & (X (! m)))) U (((m & (! LAST)) & (X (! m))) & (X p))) | (LAST V (! ((m & (! LAST)) & (X (! m))))))');
});

//------------------------------------------------------------------
test('ASSISTANT TAB - MULTIPLE FORMALIZATIONS PAST LUSTRE', async () => {
     console.log('starting test '+numTest+':  ASSISTANT TAB - MULTIPLE FORMALIZATIONS PAST LUSTRE')
     ...
     var formatMenu =  window.locator('#qa_crtAst_sem_sel_past');
     await formatMenu.click();
     formatMenu =  window.locator('#qa_crtAst_sem_mi_pastLustre');
     await formatMenu.click();

     const pastTimeFormula = await window.locator('#qa_crtAst_sem_typ_pastTimeFormula');
     reqText = await pastTimeFormula.textContent();
     expect(reqText).toContain('((O((( not m) and (YtoPre(m))) and YtoPre(H( not (( not m) and (YtoPre(m))))))) => (SI( ((( not m) and (YtoPre(m))) and YtoPre(H( not (( not m) and (YtoPre(m)))))), (((( not m) and (YtoPre(m))) and YtoPre(H( not (( not m) and (YtoPre(m)))))) => p) )))');
...
// parser crashes parsing requirement containing variable name T
test('Jira - J293', async () => {
    ...
    await slateEditable.fill('NLGuidance shall always satisfy mag(yout - pre_yout) <= T * mag(Vt + Vv)');
    ...
    var reqAccordian =  window.locator('#qa_ltlSim_ib_as_reqs');
    await reqAccordian.click();

    const reqDetails  =  window.locator('#qa_ltlSim_typ_reqId');
    const reqHTML = await reqDetails.innerHTML()
    const reqId = parse(reqHTML)
    const reqIdString = reqId.toString();
    //console.log('reqIdString: ',reqIdString);
    expect(reqIdString).toContain('NLGuidance shall always satisfy mag(yout - pre_yout) <= T * mag(Vt + Vv)');

});


test('Jira - J295', async () => {
    ...
    const reqId = parse(reqHTML)
    const reqIdString = reqId.toString();
    console.log('reqIdString: ',reqIdString);   // reqIdString:  <b></b>: unknown-ID not in DB
    expect(reqIdString).toContain('component shall satisfy p');

});


test('Jira - J296', async () => {
    ...
    var newProjBtn = window.locator('#qa_newProj_tf_projectName');
    await newProjBtn.fill('test_J296');
    ...
    var selProject = window.locator('#qa_proj_select_test_J296');
    await selProject.click();
    ...
    var closeSim = await window.locator('#qa_ltlSim_ib_close');
    await closeSim.click();

    var reqCreate = await window.locator('#qa_crt_btn_create');
    await reqCreate.click();

    var req1 = await window.locator('#qa_cirPack_text_REQ1');
    await req1.click();
    ...
    await slateEditable.click();
    await slateEditable.fill('');
    await slateEditable.fill('component shall satisfy p1 & p2');

    await semanticsBtn.click();
    await simBtn.click();

    var pastTime = await window.locator('#qa_ltlSim_ib_pastFutureTime');
    await pastTime.click();

    var timeSeries =  window.locator('.recharts-surface');
    //console.log('num traces: ', await timeSeries.nth(0).allTextContents()); // time ticks
    //console.log('num traces: ', await timeSeries.nth(1).allTextContents()); // p1
    //console.log('num traces: ', await timeSeries.nth(2).allTextContents()); // p2
    //console.log('num traces: ', await timeSeries.nth(3).allTextContents()); // bug will have p, should be REQ1
    //console.log('num traces: ', await timeSeries.nth(4).allTextContents());
    expect(await timeSeries.nth(3).allTextContents()).toContain('FALSETRUEREQ1');

});


test('Jira - J298', async () => {
    ...
    await delay(5000);   // just to see the quotes
