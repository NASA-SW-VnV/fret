...
createSemantics(product, {sem:'mltl',in:'afterUntil'},
		{mltlExpanded: 'mltlExpanded', R2U2Code: 'R2U2Code'});
	    ...
	    if (sem) { // SALT did not return undefined
		      if (sem.startsWith('LTLSPEC')) {
                  ...
                  if (options.sem === 'mltl'){
                    ...
                    let ftExpCustOptMLTLStream = formalizations.translateToMLTL(xform.transformToStream(ftExpCustOpt));
                    FRETSemantics[key][properties.R2U2Code] = ftExpCustOptMLTLStream;
                    break;
                  }
...
function createSaltBatchString(product,options) {
  ...
  while (!iterator.done) {
    ...
    var endpoints = {left:'', right:'',
      ...
      endpoints['SMVftExtleft2'] = semanticsGenerator.customizeForFret(formalizations.EndPointsRewrite2('ft', eps[0], 'smv'));
      endpoints['SMVftExtright2'] = semanticsGenerator.customizeForFret(formalizations.EndPointsRewrite2('ft', eps[1], 'smv'));
