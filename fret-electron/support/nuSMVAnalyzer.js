...
nuSMVAnalyzer.prototype.visitUnaryPastOp = function(ctx) {
  var operator = antlrUtilities.getText(ctx.pastUnaryOp(0));
  if (operator === 'Y'){
    operator = 'pre ';
  }
...
nuSMVAnalyzer.prototype.visitTimedUnarySaltPastOp = function(ctx) {
  ...
  result = result + 'T(' + saltBound + ','+antlrUtilities.getText(ctx.ltlExpr(0)) + ')';
...
// Visit a parse tree produced by NuSMVParser#binaryOp.
 nuSMVAnalyzer.prototype.visitBinaryPastOp = function(ctx) {
   var result = antlrUtilities.getText(ctx.pastBinaryOp(0))+'( '+ this.visit(ctx.ltlExpr(1)) +', '+ this.visit(ctx.ltlExpr(0)) +' )';
   return result;
};
